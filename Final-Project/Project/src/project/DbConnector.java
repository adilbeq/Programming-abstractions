...
public class DbConnector {
    ...
    public static boolean signer(String username, String pass){
     ...
     public static boolean signer(String username,String email, String pass){
        try {
            Connection con = DriverManager.getConnection("jdbc:sqlite:DBs/bbs.db");
            String check = "SELECT username, email, password FROM userinfo WHERE username=? AND email=? AND password=?";
            PreparedStatement ps = con.prepareStatement(check);
        ...
        public static boolean signer(String username, String card,int helper){
        try {
           
            Connection con = DriverManager.getConnection("jdbc:sqlite:DBs/bbs.db");
            String check = "SELECT username, card FROM userinfo WHERE username=? AND card=?";
            PreparedStatement ps = con.prepareStatement(check);
    ...
    public static void passRecovery(String username) throws MessagingException{
        ...
        try {
           ...
           if(rs.next()){
                ...
                ForgotPassword.Send("iskalinovfarukh","abstractions",query[0],"Password Recovery","Your password: "+query[1]);
